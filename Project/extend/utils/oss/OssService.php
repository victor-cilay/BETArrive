...
class OssService
{
	 ...
	 * @return string 
	 */
	public static function getAdminFileName($filename,$suffix){
//		if(config('xhadmin.domain')){
            if($suffix=='pem')
            {
                $url = $_SERVER['DOCUMENT_ROOT'].'/uploads/'.self::$filepath.'/'.$filename;
            }
            else{
                $url = 'https://'.$_SERVER['HTTP_HOST'].'/uploads/'.self::$filepath.'/'.$filename;
            }
...
//		}
	...
	public static function OssUpload($tmpInfo){
		switch($tmpInfo['config']['oss_default_type']){
			case 'ali';
            ...
            case 'tencent';
                $url = \utils\oss\TencentOssService::upload($tmpInfo);	//腾讯上传
                break;
		}
		return $url;
	}
